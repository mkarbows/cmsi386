...
let rec intsFromTo ((m,n) : int * int) : int list =
  ...
  | _ when m = n -> [m]
  | _ when n-m = 1 -> [m;n]
  | m -> let result = intsFromTo ((m+1), n) in m::result;;
;;                    
...
let rec insort (l : int list) : int list =
  ...
  let rec insert (e,l) =
    match l with
    | [] -> e::l
    | hd::[] -> if e > hd then hd::e::[] else e::hd::[]
    | hd::tl -> if e > hd then hd::insert (e,tl) 
                else e::l in
  match l with 
  | []  -> []
...
let rec splitBy ((f,l) : ('a -> bool) * ('a list)) : ('a list * 'a list) =
  ...
  match l with
  | [] -> ([],[])
  | hd::tl -> let (tls, tls') = splitBy (f,tl)
        in if f hd
        then (hd::tls,tls')
        else (tls, hd::tls')
;;                        
...
let rec update ((l,n,f,default) : 'a list * int * ('a -> 'a) * 'a) =
  ...
  match (l, n) with
  |([], 0) -> [f default]
  |([], _) -> default::update ([], n-1, f, default)
  |(x::tl, 0) -> f x::tl
  |(hd::tl, _) -> hd::update (tl, n-1, f, default)
;;
...
let rec partitionBy ((f,l) : ('a -> int) * ('a list)) : ('a list) list =
  match (f,l) with
  |[] -> []
  |hd::tl -> let updatePartition (l1: ('a list)) : ('a list) = hd::l1 in update(partitionBy(f, tl), f hd, updatePartition, [])
;;
